...
跨平台 Office 命令行工具。支持文字（Writer）、表格（Calc）和演示（Impress）项目管理。

本地来源：`/Users/hanyueyang/.codex/vendor/harness-anything-mac`

## 前置条件

- **Python 3.10+**
- **macOS/Linux 导出**：LibreOffice，macOS 可使用 `brew install --cask libreoffice`
- **Windows 导出**：WPS Office 或 Microsoft Office，并安装 `pywin32`

注意：当前上游版本在 macOS 上不会直接调用 Mac 版 WPS。未安装 LibreOffice 时，可以使用项目数据编辑命令，但不要声称已完成真实 Office 文件导出。

## 安装

```bash
uv tool install --editable /Users/hanyueyang/.codex/vendor/harness-anything-mac
# 验证
